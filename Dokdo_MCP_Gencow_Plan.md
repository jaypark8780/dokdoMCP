...
| 1.3.0 | 2026-09-21 | source registry, ingest job, item-level rights review와 HTTPS allowlist 기반 수집 기반 구현 |
...
일본 정부·기관의 독도 영유권 주장처럼 검수 결과 `contested` 또는 `refuted`로 분류된 공식 입장 자료는 MCP에서 단독으로 반환하지 않는다. `contradicts` 관계의 검수된 fragment를 `claimContexts.rebuttalEvidence`에 함께 표시하며, 반박자료가 준비되지 않은 주장 자료는 공개 상태로 전환하지 않는다. 국가만으로 자동 판정하지 않고 주장별 역사·법률 사료와 검수 기록을 근거로 평가한다.
