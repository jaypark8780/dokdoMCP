...
2026-09-21 공개자료 탐색 1차분은 `data/source-candidates-20260921.json`에 기록했다. 이 파일은 공개 카탈로그에서 확인한 후보 목록이며, 권리·역사·주장 맥락 검수 전에는 MCP 공개 데이터로 간주하지 않는다.
...
공식 입장 자료는 단독 사실처럼 노출하지 않는다. 특히 일본 정부·기관이 독도를 일본 영토라고 주장하는 자료는 원문을 보존하되, 검수된 반박 사료가 연결되기 전에는 `published`로 전환하지 않는다. 공개 시에는 주장 바로 뒤에 한국과 가능한 제3국의 1차 사료·문서 fragment를 함께 표시한다.

다만 일본 자료라는 이유만으로 자동 `refuted` 판정을 부여하지 않는다. 주장 문장, 주장 주체, 검수된 반박 fragment와 평가 근거를 사람이 확인해 `contested` 또는 `refuted` 상태를 승인한다. 이를 통해 원문을 왜곡하지 않으면서도 허구·오류 주장이 맥락 없이 재유통되는 것을 방지한다.
...
- 영유권 주장 페이지는 `claims`에 연결하고, `contradicts` 관계의 검수된 반박 fragment가 최소 1개 이상 있어야 MCP에 공개한다.
- 가능하면 대한민국 정부의 설명만 연결하지 않고 관찬 사료, 조약·외교문서, 제3국 기록을 함께 제시한다.
...
5. **주장 검수:** 영유권 주장에 검수된 반박·지지·맥락 사료와 정확한 locator가 연결됐는가

### 주장과 반박자료 표시 규칙

- 원문 주장 문장을 임의로 고치거나 삭제하지 않는다.
- `claimant_country`, `claimant_institution`, `claim_type`을 명확히 표시한다.
- `assessment_status`는 `unreviewed`, `contested`, `refuted`, `substantiated` 중 하나로 관리한다.
- `refuted` 판정에는 최소 1개의 published `contradicts` fragment와 평가 요약이 필요하다.
- 일본 측 공식 영유권 주장 자료가 검색되면 MCP 응답의 `claimContexts.rebuttalEvidence`를 함께 반환한다.
- 반박자료에는 source ID, 기관, 국가, 자료유형, 원본 URL, 페이지·이미지 번호·타임코드와 인용문을 포함한다.
- 한국 정부 입장만을 반복하는 자료보다 한국 1차 사료와 제3국 기록을 우선 연결한다.
- 반박 사료가 준비되지 않은 주장은 관리자 검수 화면에만 남기고 공개 MCP에서는 제외한다.
...
- 기계 생성 필드와 생성 방법
- 공식 주장인 경우 `claim_id`, 평가 상태, 반박·지지·맥락 evidence 관계
