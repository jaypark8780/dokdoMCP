...
const LOGIN_BOOTSTRAP = `<script>document.addEventListener('DOMContentLoaded',()=>{document.body.innerHTML='<main style="max-width:440px;margin:10vh auto;padding:28px;background:white;font-family:system-ui;border:1px solid #e0e5ef;border-radius:18px"><h1>Dokdo MCP Admin</h1><p>Sign in or create the approved administrator account.</p><label>Email</label><input id="admin-email" type="email" value="jaypark8780@gmail.com" style="display:block;width:100%;box-sizing:border-box;padding:12px;margin:6px 0 14px"><label>Password</label><input id="admin-password" type="password" style="display:block;width:100%;box-sizing:border-box;padding:12px;margin:6px 0 14px"><button id="admin-signin" style="width:100%;padding:12px">Sign in</button><button id="admin-signup" style="width:100%;padding:12px;margin-top:10px">Create the first admin account</button><p id="admin-message" style="color:#b42318;min-height:24px"></p><small>No email is sent. Passwords are handled by Gencow authentication.</small></main>';const e=document.querySelector('#admin-email'),p=document.querySelector('#admin-password'),m=document.querySelector('#admin-message');const send=async path=>{if(!e.value||!p.value){m.textContent='Enter email and password.';return}m.textContent='Working…';try{const r=await fetch(path,{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({email:e.value.trim(),password:p.value,name:'Jay Park'})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.message||d.error||('Request failed ('+r.status+')'));location.href='/dashboard'}catch(x){m.textContent=x.message}};document.querySelector('#admin-signin').onclick=()=>send('/api/auth/sign-in/email');document.querySelector('#admin-signup').onclick=()=>send('/api/auth/sign-up/email')});</script>`;
