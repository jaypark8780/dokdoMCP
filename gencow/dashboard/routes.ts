...
const LOGIN_BOOTSTRAP = `<script>document.addEventListener('DOMContentLoaded',()=>{document.body.innerHTML='<main style="max-width:440px;margin:10vh auto;padding:28px;background:white;font-family:system-ui;border:1px solid #e0e5ef;border-radius:18px"><h1>Dokdo MCP Admin</h1><p>Sign in with an administrator account provisioned through Gencow. Public account creation is disabled.</p><label>Email</label><input id="email" type="email" autocomplete="email" style="display:block;width:100%;box-sizing:border-box;padding:12px;margin:6px 0 14px"><label>Password</label><input id="password" type="password" autocomplete="current-password" style="display:block;width:100%;box-sizing:border-box;padding:12px;margin:6px 0 14px"><button id="form" type="button" style="width:100%;padding:12px">Sign in</button><p id="msg" style="color:#b42318;min-height:24px"></p><small>Ask the deployment administrator to add your email to DOKDO_ADMIN_EMAILS.</small></main>';const e=document.querySelector('#email'),p=document.querySelector('#password'),m=document.querySelector('#msg');const send=async()=>{if(!e.value.trim()||!p.value){m.textContent='Enter email and password.';return}m.textContent='Working…';try{const r=await fetch('/api/auth/sign-in/email',{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({email:e.value.trim(),password:p.value})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.message||d.error||('Request failed ('+r.status+')'));location.href='/dashboard'}catch(x){m.textContent=x.message}};document.querySelector('#form').onclick=send});</script>`;
