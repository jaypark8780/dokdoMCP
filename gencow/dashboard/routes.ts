...
import { isDashboardAdminEmail, requireDashboardAdmin } from "../authz";

// The login page is intentionally anonymous; only the dashboard and summary enforce the admin allowlist.

const LOGIN_HTML = String.raw`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Dokdo MCP Admin sign in</title><style>body{margin:0;background:#f5f7fb;color:#172033;font-family:Inter,system-ui,sans-serif}.box{max-width:440px;margin:10vh auto;padding:28px;background:#fff;border:1px solid #e0e5ef;border-radius:18px;box-shadow:0 8px 30px #20335c12}h1{margin:0 0 8px}p{color:#5c667a;line-height:1.5}label{display:block;margin:16px 0 6px;font-weight:600}input{box-sizing:border-box;width:100%;padding:12px;border:1px solid #cbd3e1;border-radius:9px;font-size:1rem}button{width:100%;padding:12px;margin-top:18px;border:0;border-radius:9px;background:#315ee7;color:#fff;font-size:1rem;font-weight:700;cursor:pointer}button.secondary{background:#eef1f6;color:#172033;margin-top:10px}.msg{min-height:24px;color:#b42318;margin-top:14px}.note{font-size:.85rem;color:#68738a}.lang{font-size:.9rem}</style></head><body><main class="box"><div class="lang">DOKDO MCP · REVIEW CONSOLE</div><h1>Admin sign in</h1><p>Sign in to review historical sources, rights, and claim evidence. Only the approved administrator email can access this dashboard.</p><form id="form"><label for="email">Email</label><input id="email" type="email" value="jaypark8780@gmail.com" required autocomplete="email"><label for="password">Password</label><input id="password" type="password" required autocomplete="current-password"><button>Sign in</button></form><button class="secondary" id="signup">Create the first admin account</button><div id="msg" class="msg"></div><p class="note">Your password is sent directly to Gencow authentication and is not shown to or stored by this application.</p></main><script>const msg=document.querySelector('#msg');const email=()=>document.querySelector('#email').value.trim();const password=()=>document.querySelector('#password').value;async function send(path){msg.textContent='Working…';const r=await fetch(path,{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({email:email(),password:password(),name:'Jay Park'})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||d.error||('Request failed ('+r.status+')'));location.href='/dashboard'}document.querySelector('#form').addEventListener('submit',e=>{e.preventDefault();send('/api/auth/sign-in/email').catch(e=>msg.textContent=e.message)});document.querySelector('#signup').addEventListener('click',()=>{if(!isAllowed())return;send('/api/auth/sign-up/email').catch(e=>msg.textContent=e.message)});function isAllowed(){if(email().toLowerCase()!=='jaypark8780@gmail.com'){msg.textContent='Use the approved administrator email.';return false}return true}</script></body></html>`;

const LOGIN_BOOTSTRAP = `<script>document.addEventListener('DOMContentLoaded',()=>{document.body.innerHTML='<main style="max-width:440px;margin:10vh auto;padding:28px;background:white;font-family:system-ui;border:1px solid #e0e5ef;border-radius:18px"><h1>Dokdo MCP Admin</h1><p>Sign in or create the approved administrator account.</p><label>Email</label><input id="email" type="email" value="jaypark8780@gmail.com" style="display:block;width:100%;box-sizing:border-box;padding:12px;margin:6px 0 14px"><label>Password</label><input id="password" type="password" style="display:block;width:100%;box-sizing:border-box;padding:12px;margin:6px 0 14px"><button id="form" type="button" style="width:100%;padding:12px">Sign in</button><button id="signup" type="button" style="width:100%;padding:12px;margin-top:10px">Create the first admin account</button><p id="msg" style="color:#b42318;min-height:24px"></p><small>No email is sent. Passwords are handled by Gencow authentication.</small></main>';const e=document.querySelector('#email'),p=document.querySelector('#password'),m=document.querySelector('#msg');const send=async path=>{if(e.value.trim().toLowerCase()!=='jaypark8780@gmail.com'){m.textContent='Use the approved administrator email.';return}if(!p.value){m.textContent='Enter a password.';return}m.textContent='Working…';try{const r=await fetch(path,{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({email:e.value.trim(),password:p.value,name:'Jay Park'})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.message||d.error||('Request failed ('+r.status+')'));location.href='/dashboard'}catch(x){m.textContent=x.message}};document.querySelector('#form').onclick=()=>send('/api/auth/sign-in/email');document.querySelector('#signup').onclick=()=>send('/api/auth/sign-up/email')});</script>`;

export const loginPageRoute = httpRoute.get
  .path("/login")
  .allowAnonymous()
  .handler(async () => ({ headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" }, body: LOGIN_BOOTSTRAP + LOGIN_HTML }));
...
export const dashboardSummaryRoute = httpRoute.get
  ...
  .handler(async ({ context: ctx }) => {
    ...
    const queueRows = await ctx.db
      ...
      .where(and(eq(claims.reviewStatus, "pending"), eq(claimLocalizations.reviewStatus, "published")))
      .limit(20);
