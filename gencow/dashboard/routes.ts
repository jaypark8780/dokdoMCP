...
const LOGIN_BOOTSTRAP = `<script>document.addEventListener('DOMContentLoaded',()=>{const f=document.querySelector('form'),e=document.querySelector('input[type=email]'),p=document.querySelector('input[type=password]'),m=document.querySelector('.msg'),b=document.querySelector('button.secondary');const send=async path=>{if(!e.value||!p.value){m.textContent='Enter email and password.';return}m.textContent='Working…';try{const r=await fetch(path,{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({email:e.value.trim(),password:p.value,name:'Jay Park'})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.message||d.error||('Request failed ('+r.status+')'));location.href='/dashboard'}catch(x){m.textContent=x.message}};f?.addEventListener('submit',x=>{x.preventDefault();send('/api/auth/sign-in/email')});b?.addEventListener('click',()=>send('/api/auth/sign-up/email'))});</script>`;
